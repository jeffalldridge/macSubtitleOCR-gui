...
enum ConversionRunner {
    ...
    static func loadStream(for track: QueueTrack, in file: QueueFile, cache: StreamCache) async throws -> any SubtitleStream {
        ...
        do {
            // A child task, not a detached one. Extraction checks
            // `Task.isCancelled` between clusters, and a detached task never
            // sees the flag — Cancel would do nothing at all while a large
            // MKV was being read.
            let work = Task(priority: .userInitiated) { () throws -> any SubtitleStream in
                    ...
                    if source.isContainer {
                        ...
                        try Task.checkCancellation()
                        if (try? cache.store(extracted, key: key)) != nil,
            ...
            let stream = try await work.value
            track.stream = stream
